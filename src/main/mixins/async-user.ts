...
function modifyModel(model) {
	...
	model.generateVerificationToken = function(user, options, cb?) {
		if(cb) return oldModelStatic.generateVerificationToken.call(this, user, options, cb)
		else {
	...
	model.login = function(credentials, include, cb?) {
		if(cb) return oldModelStatic.login.call(this, credentials, include, cb)
		else {
			return new Promise((resolve, reject) => {
	...
	model.logout = function(accessTokenID, cb?) {
		if(cb) return oldModelStatic.logout.call(this, accessTokenID, cb)
		else {
			return new Promise((resolve, reject) => {
